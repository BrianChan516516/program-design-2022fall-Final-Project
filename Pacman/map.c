...
/*global variables*/
// [ NOTE ]
const int block_width = 32,  block_height = 32;			// the pixel size of a "block"
...
static Button speed_up;
static Button speed_down;
static Button milk;
...
void delete_map(Map* M) {
	...
	*/
    if (M->map) {
        ...
        free(M->map);
    } // one by one then free the whole
    free(M);
}
		...
		for (int col = 0; col < M->col_num; col++) {
			switch (M->map[row][col])
			{
				...
				case '.':
					...
					break;
                case 'S':
                    speed_up = button_create(map_offset_x + col * block_width + block_width / 2.0 - 20, map_offset_y + row * block_height + block_height / 2.0 - 20, 40, 40, "Assets/speed_bean.png", "Assets/speed_bean.png");
                    printf("row: %d col: %d\n",row,col);
                    drawButton(speed_up);
                    break;
                case 'D':
                    speed_down = button_create(map_offset_x + col * block_width + block_width / 2.0 - 20, map_offset_y + row * block_height + block_height / 2.0 - 20, 40, 40, "Assets/ice.png", "Assets/ice.png");
                    printf("row: %d col: %d\n",row,col);
                    drawButton(speed_down);
                    break;
                case 'M':
                    milk = button_create(map_offset_x + col * block_width + block_width / 2.0 - 20, map_offset_y + row * block_height + block_height / 2.0 - 20, 40, 40, "Assets/milk.png", "Assets/milk.png");
                    printf("row: %d col: %d\n",row,col);
                    drawButton(milk);
                    break;
				default:
...
static void draw_block_index(Map* M, const int row, const int col) {
	...
	dw = block_width / 3;
		...
		al_draw_filled_rectangle(s_x, s_y, e_x, e_y, al_map_rgb(61,86,171));
...
static void draw_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width/6.0,  al_map_rgb(255, 127, 80));
}

static void draw_power_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width / 3.0, al_map_rgb(255, 255, 190));
}
