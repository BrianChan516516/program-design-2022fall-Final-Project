...
static void ghost_red_move_script_FREEDOM(Ghost* ghost, Map* M, Pacman* pacman) {
    ...
    if(ghost->typeFlag == Babb) {
        Directions shortest = shortest_path_direc(M, ghost->objData.Coord.x, ghost->objData.Coord.y, pacman->objData.Coord.x, pacman->objData.Coord.y);
        ghost_NextMove(ghost,shortest);
        return;
    }
    ...
    for (Directions i = 1; i <= 4; i++) {
        if(ghost_movable(ghost, M, i, true)) {
            if((i == UP && ghost->objData.facing != DOWN) ||
               (i == DOWN && ghost->objData.facing != UP) ||
               (i == LEFT && ghost->objData.facing != RIGHT) ||
               (i == RIGHT && ghost->objData.facing != LEFT))  proba[cnt++] = i;
        }
    }
    //if (ghost_movable(ghost, M, i, true) && (ghost->objData.facing+i!=5)) proba[cnt++] = i;
    if(cnt)ghost_NextMove(ghost, proba[generateRandomNumber(0, cnt - 1)%4]);
